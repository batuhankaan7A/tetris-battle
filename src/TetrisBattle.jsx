...
const compressBoard = (board) => board.map(row => row.map(c => c ? 1 : 0));
...
export default function TetrisBattle() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => { setMyId(socket.id); socket.emit("get_rooms"); });
    socket.on("room_list", setRoomList);
    socket.on("room_update", (data) => { setRoomData(data); if (screen === "lobby") setScreen("room"); });
    socket.on("game_start", ({ players }) => {
      setOtherPlayers(Object.fromEntries(Object.entries(players).filter(([id]) => id !== socket.id).map(([id, p]) => [id, { ...p, board: null }])));
      resetGame();
      setEliminated(false);
      setScreen("game");
    });
    socket.on("player_update", ({ id, board }) => {
      setOtherPlayers(prev => ({ ...prev, [id]: { ...prev[id], board } }));
    });
    socket.on("player_eliminated", ({ id }) => {
      if (id === socket.id) setEliminated(true);
      setOtherPlayers(prev => ({ ...prev, [id]: { ...prev[id], alive: false } }));
    });
    ...
    socket.on("error_msg", (msg) => { setErrorMsg(msg); setTimeout(() => setErrorMsg(""), 3000); });
  ...
  const render = useCallback(() => {
    ...
    ctx.fillStyle = "#07070e"; ctx.fillRect(0, 0, COLS*B, ROWS*B);
    ...
    if (nc) {
      ...
      if (g.nxt) {
        ...
        g.nxt.matrix.forEach((row,r)=>row.forEach((v,c)=>{ if(v){nctx.fillStyle=g.nxt.color;nctx.fillRect((ox+c)*NB+2,(oy+r)*NB+2,NB-3,NB-3);} }));
      }
    }
  }, [BS]);
  ...
  const place = useCallback(() => {
    ...
    socketRef.current?.emit("game_update", { board: compressBoard(g.board) });
  }, [endGame]);
  ...
  const tick = useCallback((t=0) => {
    ...
    if (newLevel !== g.level) { g.level = newLevel; setUi(u=>({...u, level:g.level})); }
    g.dropT += delta;
  ...
  const resetGame = useCallback(() => {
    ...
    g.raf = requestAnimationFrame((t) => { g.gameStartTime = t; g.lastT = t; tick(t); });
  }, [tick]);
  ...
  const act = useCallback((action) => {
    ...
    switch(action) {
      ...
      case "T": {
        ...
        for(const k of [0,1,-1,2,-2]){ if(fits(g.board,g.cur,k,0,rot)){g.cur.matrix=rot;g.cur.x+=k;break;} }
        break;
      }
      case "DROP": {
        let dy=0; while(fits(g.board,g.cur,0,dy+1))dy++;
  ...
  const startDown = useCallback((e) => {
    e.preventDefault(); stopDown(); act("D");
  ...
  const MiniBoard = ({ board, alive, nickname }) => {
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,opacity:alive===false?0.35:1}}>
        <div style={{fontSize:8,color:"#ffffff88",maxWidth:50,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{nickname}</div>
        <div style={{width:COLS*MB,height:ROWS*MB,background:"#0a0a14",border:`1px solid ${alive===false?"#ff006e33":"#1a1a2e"}`,position:"relative",overflow:"hidden"}}>
          ...
          {alive === false && (
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.6)",fontSize:8,color:"#ff006e",fontFamily:"'Orbitron',monospace"}}>ELENDİ</div>
          )}
        </div>
      </div>
    );
  ...
  const inputStyle = {background:"rgba(15,15,26,0.9)",border:"1px solid #1a1a2e",color:"#e0e0ff",fontFamily:"'Share Tech Mono',monospace",fontSize:14,padding:"10px 14px",borderRadius:8,outline:"none",width:"100%"};
  const actionBtnStyle = (color="#00f5ff") => ({background:"transparent",border:`1.5px solid ${color}`,color,fontFamily:"'Orbitron',monospace",fontSize:11,letterSpacing:"0.15em",padding:"12px 0",cursor:"pointer",borderRadius:8,width:"100%",boxShadow:`0 0 16px ${color}33`});
  ...
  if (screen === "lobby") return (
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Orbitron',monospace",padding:20,gap:20}}>
      ...
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:12}}>
        ...
        <button style={actionBtnStyle("#00f5ff")} onClick={()=>{ if(!nickname.trim()){setErrorMsg("Kullanıcı adı gir.");return;} if(!roomId.trim()){setErrorMsg("Oda adı gir.");return;} socketRef.current?.emit("join_room",{roomId:roomId.trim(),nickname:nickname.trim()}); }}>ODA GİR / OLUŞTUR</button>
      </div>
      {roomList.length > 0 && (
    ...
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Orbitron',monospace",padding:20,gap:16}}>
      ...
      {resultData && (
        <>
          ...
          <div style={{width:"100%",maxWidth:320}}>
            {Object.values(resultData.players).sort((a,b)=>(a.rank||99)-(b.rank||99)).map(p => (
              <div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",background:"rgba(15,15,26,0.8)",border:`1px solid ${p.rank===1?"#ffe60033":"#1a1a2e"}`,borderRadius:8,marginBottom:6,fontFamily:"'Share Tech Mono',monospace",fontSize:12,color:p.id===resultData.myId?"#00f5ff":"#e0e0ff"}}>
                ...
                <span style={{color:"#ffffff55"}}>Seviye {p.level||1}</span>
              </div>
            ))}
          </div>
        </>
      ...
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:10}}>
        {roomData?.host===myId&&<button style={actionBtnStyle("#ffe600")} onClick={()=>socketRef.current?.emit("restart_game")}>TEKRAR OYNA</button>}
  ...
  return (
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",fontFamily:"'Orbitron',monospace",paddingTop:8,overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:gameWidth,marginBottom:6}}>
        ...
        <div style={{fontSize:11,fontWeight:900,letterSpacing:"0.15em",color:"#fff"}}>TETRİS BATTLE</div>
        <div style={{textAlign:"center"}}>
      ...
      <div style={{display:"flex",gap:8,padding:"0 6px",alignItems:"flex-start"}}>
        <div style={{position:"relative",flexShrink:0}}>
          <canvas ref={canvasRef} width={COLS*B} height={ROWS*B} style={{display:"block",border:"1px solid #1a1a2e",boxShadow:"0 0 20px rgba(0,245,255,0.05)"}}/>
          {eliminated && (
          ...
          {others.length > 0 && (
            <div style={{background:"#0d0d1a",border:"1px solid #1a1a2e",padding:6,borderRadius:4}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {others.map(p => <MiniBoard key={p.id} board={p.board} alive={p.alive} nickname={p.nickname}/>)}
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{marginTop:12,display:"flex",flexDirection:"column",alignItems:"center",gap:10,width:"100%",maxWidth:320,padding:"0 12px"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <div onPointerDown={startDown} style={{width:72,height:72,borderRadius:14,fontSize:26,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(14,14,24,0.97)",border:"1.5px solid #06d6a055",color:"#06d6a0",cursor:"pointer",userSelect:"none",WebkitUserSelect:"none",touchAction:"none",boxShadow:"0 0 16px #06d6a020",fontFamily:"'Share Tech Mono',monospace",flexShrink:0}}>↓</div>
          <Btn label="→" color="#00f5ff" onPress={()=>act("R")}/>
        </div>
        <div onPointerDown={(e)=>{e.preventDefault();act("DROP");}} style={{width:"100%",height:52,borderRadius:12,fontSize:12,letterSpacing:"0.2em",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(14,14,24,0.97)",border:"1.5px solid #ff006e55",color:"#ff006e",cursor:"pointer",userSelect:"none",WebkitUserSelect:"none",touchAction:"none",fontFamily:"'Orbitron',monospace"}}>⬇ DÜŞÜR</div>
      </div>
    </div>
  );
}
