...
// Board'u sıkıştır — sadece renkler, null yerine 0
...
export default function TetrisBattle() {
  ...
  // Ekran: "lobby" | "room" | "game" | "result"
  const [screen, setScreen] = useState("lobby");
  const [nickname, setNickname] = useState("");
  const [roomId, setRoomId] = useState("");
  const [roomList, setRoomList] = useState([]);
  const [roomData, setRoomData] = useState(null); // { players, host, started }
  ...
  const [ui, setUi] = useState({score:0, level:1, lines:0});
  const [eliminated, setEliminated] = useState(false);

  // Diğer oyuncuların mini board'ları
  ...
  useEffect(() => {
    ...
    socket.on("player_update", ({ id, score, board }) => {
      setOtherPlayers(prev => ({
        ...prev,
        [id]: { ...prev[id], score, board },
      }));
    });
  ...
  const endGame = useCallback(() => {
    ...
    setEliminated(true);
    socketRef.current?.emit("game_over", { score: g.score });
  }, []);
  ...
  const place = useCallback(() => {
    ...
    if (cl) { g.score+=[0,100,300,500,800][cl]*g.level; g.lines+=cl; g.level=Math.floor(g.lines/10)+1; }
    g.cur=g.nxt; g.nxt=newPiece();
    if (!fits(g.board,g.cur)) { endGame(); return; }
    setUi(u=>({...u,score:g.score,level:g.level,lines:g.lines}));

    // Sunucuya güncelleme gönder (her yerleşimde)
  ...
  const speed = lv => Math.max(80, 500-(lv-1)*40);

  const tick = useCallback((t=0) => {
    const g = G.current;
    if (g.over) return;
    g.dropT += t - g.lastT; g.lastT = t;
  ...
  const resetGame = useCallback(() => {
    const g = G.current;
    cancelAnimationFrame(g.raf);
    Object.assign(g, {board:newBoard(),score:0,level:1,lines:0,over:false,dropT:0,lastT:0});
    g.cur = newPiece(); g.nxt = newPiece();
    setUi({score:0,level:1,lines:0});
    g.raf = requestAnimationFrame(tick);
  }, [tick]);
  ...
  const act = useCallback((action) => {
    ...
    switch(action) {
      ...
      case "D":
        if(fits(g.board,g.cur,0,1)){g.cur.y++;g.score++;g.dropT=0;}
        else{place();return;}
        break;
      case "T": {
      ...
      case "DROP": {
        let dy=0; while(fits(g.board,g.cur,0,dy+1))dy++;
        g.score+=dy*2; g.cur.y+=dy; place(); return;
      }
    }
  ...
  // Mini board önizlemesi (diğer oyuncular)
  const MiniBoard = ({ board, alive, nickname, score }) => {
    const MB = 4;
    return (
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,opacity:alive===false?0.35:1}}>
        ...
        <div style={{
          width:COLS*MB, height:ROWS*MB,
          background:"#0a0a14",
          border:`1px solid ${alive===false?"#ff006e33":"#1a1a2e"}`,
        ...
        <div style={{fontSize:9,color:"#00f5ff88"}}>{score||0}</div>
      </div>
    );
  };
  ...
  if (screen === "lobby") return (
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Orbitron',monospace",padding:20,gap:20}}>
      ...
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:12}}>
        ...
        <input
          style={inputStyle}
          placeholder="Oda adı (yeni veya mevcut)"
          value={roomId}
        ...
        <div style={{width:"100%",maxWidth:320}}>
          ...
          {roomList.map(r => (
            <div
              key={r.id}
              onPointerDown={()=>{
                if (!nickname.trim()) { setErrorMsg("Önce kullanıcı adı gir."); return; }
                setRoomId(r.id);
                socketRef.current?.emit("join_room", { roomId: r.id, nickname: nickname.trim() });
              }}
  ...
  if (screen === "room") return (
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Orbitron',monospace",padding:20,gap:16}}>
      ...
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:10}}>
        ...
        <button
          style={actionBtnStyle("#ff006e")}
          onClick={()=>{ socketRef.current?.emit("leave_room"); setScreen("lobby"); setRoomData(null); }}
        >ODADAN ÇIK</button>
      </div>
    </div>
  );
  ...
  if (screen === "result") return (
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Orbitron',monospace",padding:20,gap:16}}>
      ...
      {resultData && (
        <>
          <div style={{fontSize:13,color:"#00f5ff",marginBottom:8}}>
            🏆 {resultData.players[resultData.winner]?.nickname || "?"} kazandı!
          </div>
          <div style={{width:"100%",maxWidth:320}}>
            {Object.values(resultData.players)
              ...
              .map((p,i) => (
                <div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",background:"rgba(15,15,26,0.8)",border:`1px solid ${p.rank===1?"#ffe60033":"#1a1a2e"}`,borderRadius:8,marginBottom:6,fontFamily:"'Share Tech Mono',monospace",fontSize:12,color:p.id===resultData.myId?"#00f5ff":"#e0e0ff"}}>
                  ...
                  <span style={{color:"#ffffff55"}}>{p.score}</span>
                </div>
              ))}
          </div>
        </>
      ...
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:10}}>
        {roomData?.host === myId && (
          <button style={actionBtnStyle("#ffe600")} onClick={()=>socketRef.current?.emit("restart_game")}>
            TEKRAR OYNA
          </button>
        )}
        <button style={actionBtnStyle("#ff006e")} onClick={()=>{ socketRef.current?.emit("leave_room"); setScreen("lobby"); setRoomData(null); }}>
          LOBIYE DÖN
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{background:"#07070e",minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",fontFamily:"'Orbitron',monospace",paddingTop:8,overflow:"hidden"}}>
      ...
      {/* Üst bar */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",maxWidth:420,padding:"0 10px",marginBottom:6}}>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:8,color:"#00f5ff88",letterSpacing:"0.15em"}}>SKOR</div>
          <div style={{fontSize:18,fontWeight:900,color:"#00f5ff",textShadow:"0 0 10px #00f5ff88"}}>{ui.score}</div>
        </div>
        <div style={{fontSize:13,fontWeight:900,letterSpacing:"0.2em",color:"#fff"}}>TETRİS BATTLE</div>
        <div style={{display:"flex",gap:10}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:8,color:"#8338ec88"}}>SEV</div>
            <div style={{fontSize:16,fontWeight:700,color:"#8338ec"}}>{ui.level}</div>
          </div>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:8,color:"#06d6a088"}}>SATIR</div>
            <div style={{fontSize:16,fontWeight:700,color:"#06d6a0"}}>{ui.lines}</div>
          </div>
        </div>
      </div>

      {/* Oyun + yan panel */}
      <div style={{display:"flex",gap:8,padding:"0 6px",alignItems:"flex-start",width:"100%",maxWidth:420}}>
        {/* Ana board */}
        ...
        {/* Sağ panel: sonraki + diğer oyuncular */}
          ...
          {others.length > 0 && (
            <div style={{background:"#0d0d1a",border:"1px solid #1a1a2e",padding:6,borderRadius:4}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {others.map(p => (
                  <MiniBoard key={p.id} board={p.board} alive={p.alive} nickname={p.nickname} score={p.score}/>
                ))}
              </div>
            </div>
          )}
